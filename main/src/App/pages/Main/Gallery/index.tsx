            ...
            <ResponsiveMasonry columnsCountBreakPoints={{ 350: 1, 768: 2 }}>
                <Masonry containerTag="section" className="!gap-6 [&>div]:!gap-6">
                    {data.map((image: any, i: number) => (
                        <AdvancedImage className="block w-full rounded shadow-md" cldImg={getImageURL(image)} plugins={[lazyload({ rootMargin: '350px 0px 0px 0px', threshold: 0 }), responsive()]} key={i} />
                    ))}
                </Masonry>
            </ResponsiveMasonry>
            <hr className="mt-4" />
